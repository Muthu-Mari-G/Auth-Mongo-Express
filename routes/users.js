...
router.get("/api/getUsers/", async (req, res) => {
    try {
        ...
        if (parsedLimit > totalUsers) {
            return res.status(404).json({ message: `There are only ${totalUsers} users` });
        }
...
router.get("/api/getbyidUsers/:id", async (req, res) => {
    try {
...
router.delete("/api/users/:id", async (req, res) => {
    const deletedUser = await Card.findByIdAndDelete(req.params.id);
